...
def add_tag_to_list_of_media_route_handler(data):
    try:
        ...
        tag_name = data.get('tag') if data else None
        ...
        with db_connection(commit=True) as cursor:
            tag = select_tag_by_name(cursor, tag_name)
            
            if not tag:
                return {"error": "Tag not found", 'tag': tag_name}, 404
        
            rows_updated = insert_Xref_Tag2Media_bulk_to_db(cursor, media_ids, tag['id'])
            if rows_updated == 0:
                return {"error": "Tag already associated with all specified media", 'tag_id': tag['id']}, 400
            
            return {"message": f"Tag associated with {rows_updated} media items successfully", 'tag_id': tag['id'], 'updated_count': rows_updated}, 200
